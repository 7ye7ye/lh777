          ...
          <view v-else class="time-slots">
           <view
             ...
             :class="{
               ...
               disabled: !slotStatus[slot.key]?.available
             }"
           ...
           >
             <view class="slot-info">
               ...
               <text
                 v-if="slotStatus[slot.key]?.available && slotStatus[slot.key]?.remaining > 0"
                 class="slot-quota"
               >
             ...
             <!-- 精准状态显示：无排班 / 已满 / 已选择 -->
             <view
               v-if="appointmentDate && !slotStatus[slot.key]?.available && slotStatus[slot.key]?.remaining === 0"
               class="slot-status none"
             >
               无号
             </view>
             ...
             <view
               v-else-if="slotStatus[slot.key]?.remaining <= 0"
               class="slot-status full"
             >
               已满
             </view>
           
             <view
               v-else-if="selectedSlot === slot.key"
               class="slot-status selected"
             >
               已选择
             </view>
           </view>
...
const timeSlots = [
  { id: 1, label: '上午', timeRange: '08:00-12:00', key: 'morning' },
  { id: 2, label: '下午', timeRange: '14:00-17:00', key: 'afternoon' },
  { id: 3, label: '晚上', timeRange: '18:00-20:00', key: 'evening' }
]

const timeSlotMap = { morning: 1, afternoon: 2, evening: 3 }
const timeSlotReverseMap = { 1: 'morning', 2: 'afternoon', 3: 'evening' }
...
const canSubmit = computed(() => {
  ...
  const availableQuota = Number(selectedSchedule.value.available_quota ?? selectedSchedule.value.availableQuota ?? 0)
...
const loadRegistrationTypes = async () => {
  try {
    ...
    console.error(error)
    uni.showToast({ title: '加载挂号类型失败', icon: 'none' })
  }
}
...
const loadSchedules = async (doctorId, startDate, days = 7) => {
  ...
  try {
    ...
    if (!scheduleList.length) {
      uni.showToast({ title: '该医生暂无排班', icon: 'none' })
    }
    ...
    schedules.value = []
    uni.showToast({ title: '加载排班信息失败', icon: 'none' })
  } finally {
...
const slotStatus = computed(() => {
  const result = {
    ...
    evening: { available: false, remaining: 0 }
  }
...
const selectTimeSlot = (slot) => {
  const slotInfo = slotStatus.value[slot.key] || { available: false, remaining: 0 }

  if (!slotInfo.available && slotInfo.remaining <= 0) {
    uni.showToast({ title: '该时段不可选', icon: 'none' })
    return
  }
...
const confirmAppointment = async () => {
  if (!canSubmit.value) {
    uni.showToast({ title: '请完整选择预约信息', icon: 'none' })
    return
  }
...
.name-title { display: flex; align-items: center; margin-bottom: 12rpx; }
.name { font-size: 36rpx; font-weight: bold; color: #fff; margin-right: 16rpx; }
.title { font-size: 24rpx; color: #fff; background: rgba(255,255,255,0.3); padding: 4rpx 12rpx; border-radius: 4rpx; }
.specialty { font-size: 28rpx; color: rgba(255,255,255,0.9); }

.section-card { background: #fff; border-radius: 16rpx; margin: 16rpx; padding: 24rpx; box-shadow: 0 4rpx 16rpx rgba(58,156,255,0.08); }
.section-title { font-size: 32rpx; font-weight: bold; color: #333; margin-bottom: 16rpx; }
.section-content { font-size: 28rpx; color: #666; }

.picker-display { font-size: 28rpx; color: #666; padding: 12rpx 0; display: flex; justify-content: space-between; align-items: center; }
.price-hint { color: #3a9cff; font-weight: bold; }
.date-hint { font-size: 24rpx; color: #999; margin-bottom: 12rpx; }

.loading-text, .empty-text { text-align: center; padding: 40rpx 0; color: #999; font-size: 28rpx; }

.time-slots { display: flex; flex-direction: column; gap: 16rpx; }
.time-slot-item { display: flex; justify-content: space-between; align-items: center; padding: 24rpx; border: 2rpx solid #e0e0e0; border-radius: 12rpx; background: #fff; transition: all 0.3s; }
.time-slot-item.selected { border-color: #3a9cff; background: #f0f7ff; }
.time-slot-item.disabled { opacity: 0.5; background: #f5f5f5; }
.time-slot-item:active:not(.disabled) { transform: scale(0.98); }


.slot-info { display: flex; flex-direction: column; gap: 8rpx; }
.slot-time { font-size: 32rpx; font-weight: bold; color: #333; }
.slot-quota { font-size: 24rpx; color: #999; }
.slot-status { font-size: 24rpx; padding: 8rpx 16rpx; border-radius: 8rpx; }
.slot-status.full { background: #ffebee; color: #f44336; }
.slot-status.selected { background: #3a9cff; color: #fff; }
.slot-status.none {
  background: #f0f0f0;
  color: #999;
}
...
.summary-card { background: linear-gradient(135deg, #f0f7ff 0%, #e8f5e9 100%); }
.summary-item { display: flex; justify-content: space-between; padding: 12rpx 0; font-size: 28rpx; }
.summary-label { color: #666; }
.summary-value { color: #333; font-weight: bold; }
.summary-value.price { color: #f44336; font-size: 32rpx; }

.action-buttons { position: fixed; bottom: 0; left: 0; right: 0; display: flex; padding: 16rpx; background: #fff; box-shadow: 0 -4rpx 16rpx rgba(0,0,0,0.05); }
.action-btn { flex: 1; display: flex; align-items: center; justify-content: center; padding: 20rpx; border-radius: 12rpx; background: linear-gradient(135deg, #3a9cff 0%, #1de9b6 100%); color: #fff; font-size: 28rpx; }
.action-btn.disabled { background: #ccc; opacity: 0.6; }
.action-btn:active:not(.disabled) { opacity: 0.8; }

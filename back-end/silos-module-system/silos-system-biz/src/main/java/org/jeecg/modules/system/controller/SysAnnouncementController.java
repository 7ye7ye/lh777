...
public class SysAnnouncementController {
    ...
    public Result<Map<String, Object>> listByUser(@RequestParam(required = false, defaultValue = "5") Integer pageSize, HttpServletRequest request) {
        ...
        // 安全获取当前登录用户信息 - 修复类型转换问题
        LoginUser sysUser = getCurrentUserSafely();
        if (sysUser == null) {
            result.error500("用户未登录或用户信息获取失败");
            return result;
        }
...
//		});
    ...
    /**
     * 安全获取当前登录用户信息
     * 处理 HosUser 到 LoginUser 的类型转换问题
     */
    private LoginUser getCurrentUserSafely() {
        Object principal = SecurityUtils.getSubject().getPrincipal();

        if (principal instanceof LoginUser) {
            return (LoginUser) principal;
        } else if (principal instanceof HosUser) {
            // 将 HosUser 转换为 LoginUser
            HosUser hosUser = (HosUser) principal;
            LoginUser loginUser = new LoginUser();
            loginUser.setId(String.valueOf(hosUser.getUserId()));
            loginUser.setUsername(hosUser.getUserAccount());
            loginUser.setRealname(hosUser.getUserAccount()); // 如果没有真实姓名，使用账号
            // 只设置 LoginUser 中存在的字段，移除不存在的 setUserType 方法
            loginUser.setPhone(hosUser.getPhone());
            loginUser.setEmail(hosUser.getEmail());
            // 如果 LoginUser 有 status 字段，保留；如果没有，注释掉
            // loginUser.setStatus(hosUser.getStatus());
            return loginUser;
        } else {
            log.warn("未知的用户类型: {}", principal != null ? principal.getClass().getName() : "null");
            return null;
        }
    }
    ...
    public ModelAndView exportXls(SysAnnouncement sysAnnouncement,HttpServletRequest request) {
        ...
        // 使用安全的方法获取用户信息
        LoginUser user = getCurrentUserSafely();
        if (user == null) {
            user = new LoginUser();
